...
public class ChatController {
    ...
    public SseEmitter chat(@RequestParam("message") String msg, @RequestHeader Map<String, String> headers) throws IOException {
        //기본 30초 시간 초과, 0L로 설정하면 시간 초과되지 않습니다
        ...
        sseEmitter.send(SseEmitter.event().id(uid).name("연결 성공！！！！").data(LocalDateTime.now()).reconnectTime(3000));
        ...
        sseEmitter.onError(
                throwable -> {
                    try {
                        ...
                        sseEmitter.send(SseEmitter.event().id("765431").name("Error Occur！").data(throwable.getMessage()).reconnectTime(3000));
                    } catch (IOException e) {
                        e.printStackTrace();
                    }
                }
